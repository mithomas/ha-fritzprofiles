...
from .fritz_profile_switch import FritzProfileSwitch, FritzProfileDevice, FritzProfileDeviceData
...
class HaFritzProfilesCoordinatorData:
    ...
    def __init__(self, fritzProfileDeviceData):
        ...
        counts = Counter(getattr(device, 'name') for device in fritzProfileDeviceData.devices)
        unique_devices_per_name = [device for device in fritzProfileDeviceData.devices if counts[getattr(device, 'name')] == 1]
        self.devices_by_name = {device.name:device for device in unique_devices_per_name}

        self.profiles_by_id = fritzProfileDeviceData.profiles_by_id
        self.profiles_by_name = {name:id for id,name in fritzProfileDeviceData.profiles_by_id.items()}
...
class HaFritzProfilesDataUpdateCoordinator(DataUpdateCoordinator): # pylint: disable=missing-class-docstring
    ...
    async def _async_update_data(self) -> FritzProfileDeviceData: # pylint: disable=missing-function-docstring
        try:
            ...
            data = HaFritzProfilesCoordinatorData(await self.hass.async_add_executor_job(self.client.load_device_profiles))
